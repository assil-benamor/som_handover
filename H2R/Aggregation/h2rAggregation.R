...
df <- read.csv("inputs/SOM1901_H2R_ClanData_December.csv", stringsAsFactors = FALSE, dec=".", sep=",", na.strings=c("NA",""," "))        
#check for and remove duplicates
deal_with_dup(df)
...
NC_depend<-read.csv("inputs/dependencies_SL_NC/h2r_NC_dep_Mar_2022.csv", stringsAsFactors = FALSE, dec=".", sep=",", na.strings=c("NA",""," "))               #import with blanks being NA's
NC_depend<-NC_depend[!is.na(NC_depend[,1]),]                                                                                                                  #remove rows that sneaked in by Excel

#import csv with SL dependencies defined
SL_depend<-read.csv("inputs/dependencies_SL_NC/h2r_SL_dep_Mar_2022.csv", stringsAsFactors = FALSE, dec=".", sep=",", na.strings=c("NA",""," "))               #import with blanks being NA's
...
NC_depend <- NC_depend %>% filter(Main.question...if.NC.the.rest.are.NC %in% names(settlement_data))
SL_depend <- SL_depend %>% filter(if_this.variable %in% names(settlement_data))
...
write.csv(settlement_data, file = paste0("outputs/SOM1901_H2R__clean_data___",agg_date,"01.csv"), na = "", row.names = FALSE)
...
write.csv(df %>% filter(info_settlement %in% settlement_level$finalsettlement),
          paste0("outputs/SOM1901_H2R_ki_level_data", agg_month,".csv"), row.names=FALSE)
