...
compute_indicators <- function(data, cluster, indicators) {
  ...
  } else if (cluster == "Inter") {
    compute_indicators_inter(data, indicators)
  }
}
...
compute_indicators_wash <- function(data, indicators) {
  ...
  data_wash_indicators <- data_wash_prep %>% mutate (
    ...
    wash_idicator1 = case_when(
      ...
      time_fetching_water %in% c("btw_16_30_minutes","more_31_minutes") & main_source_water %in% improved_water_source ~ 2,
      
      ## Water comes from an improved water source and  collection time is not more than 15 min for a roundtrip, including queuing
      time_fetching_water %in% c("less_5_minutes", "btw_5_15_minutes") & main_source_water %in% improved_water_source ~ 1,
      # 
    ...
    wash_idicator2 = case_when(
      ...
      sanitation_facility %in% no_improved_sanitation_facilities &
        ...
        (sharing_sanitation_facilities_yes_int * 7) >=20 ~ 4,
      ###YS sharing_sanitation_facilities_yes is number of hh, not number of people
      ...
      sanitation_facility %in% improved_sanitation_facilities &
        sharing_sanitation_facilities %in% c("yes") &
      ...
      sanitation_facility %in% no_improved_sanitation_facilities &
        sharing_sanitation_facilities %in% c("yes") &
        (sharing_sanitation_facilities_yes_int * 7) < 20 ~ 2,
    ...
    wash_idicator3 = case_when(
      ...
      have_soap %in% c("no") &
        hand_washing_facility %in% c("no_specific","tippy_tap") ~ 5,
    ...
    wash_idicator4 = case_when(
      ...
      drinking_water == "no"  ~ 4, #5
      
      ## EITHER enough water for drinking OR for domestic uses PLUS The HH does not have jerry cans
      (drinking_water == "yes" | (cooking_water == "no" & hygiene_water == "no") ) & currently_access_nfi.jerrycans_2 == 0 ~ 3, #4
      
      ## EITHER enough water for drinking OR for domestic uses PLUS The HH has jerry cans
      (drinking_water == "yes" & ( cooking_water == "yes" | hygiene_water == "yes") ) & currently_access_nfi.jerrycans_2 == 0 ~ 3,
    ...
    wash_idicator4 = case_when(
      ## Not enough water for drinking OR domestic uses
      drinking_water == "no" | (domestice_water == "no" & cooking_water == "no" & hygiene_water == "no") ~ 4,
      
      ## EITHER enough water for drinking OR for domestic uses PLUS The HH does not have jerry cans
      (drinking_water == "yes" | (domestice_water == "yes" & cooking_water == "yes" & hygiene_water == "yes") ) & currently_access_nfi.jerrycans_2 == 0 ~ 3,
  ...
  data_wash_indicators %>% mutate(
    wash_idicator6 = case_when(
      wash_tmp_idicator6_sum == 9 ~ 4,
      wash_tmp_idicator6_sum >= 6 ~ 3,
  ...
  max_min <- left_join(driving(res,colnames(df),T),driving(res,colnames(df),F))
  View(max_min)
  write.csv(max_min,paste0("output/max_min/",gsub("_.*$","",colnames(res)[1]),".csv"),row.names = F)
...
compute_indicators_protection <- function(data, indicators) {
  ...
  data_protection_prep <- left_join(data_protection_prep, loops_data_washington %>% mutate_at(col_names,
   ...
   # data_protection_prep %>% filter(hh_members_with_diff>0) %>% pull(hh_members_with_diff) %>% length()
...
compute_indicators_snfi <- function(data, indicators) {
  ...
  data_snfi_prep %>% mutate(
    ...
    SNFI_idicator1 = case_when(
      ...
      # how_many_shelters == 0 |
      # surface_per_person < 2 ~ 4,
      
      ## 2m2 ≤ surface per person < 2.5m2
      how_many_shelters == 0 | surface_per_person < 2 ~ 4,
      
      ## 2.5m2 ≤ surface per person < 3.5m2
      surface_per_person < 2.5 ~ 3,
...
compute_indicators_gbv <- function(data, indicators) {
  ...
  data_gbv_prep %>% mutate(
    ...
    GBV_idicator3 = case_when(
      ## No
      women_move_freely %in% c("no","do_not_know") ~ 4,
...
compute_indicators_hlp <- function(data, indicators) {
  ...
  data_hlp_prep %>% select(starts_with("land_dispute_yes.")) %>% colnames()
  ...
  data_hlp_prep %>% mutate(
    ...
    HLP_idicator1 = case_when(
      ## v.Multiple claims
      land_dispute == "yes" &
        land_dispute_yes.multiple_claims == 1 ~ 5,
...
compute_indicators_cp <- function(data, indicators) {
  ...
  children_working_colmuns <- c("permanent_job_child_male",
                                "permanent_job_child_female",
                                "temporary_job_child_male",
                                "temporary_job_child_female",
                                "daily_labor_child_male",
                                "daily_labor_child_female",
                                "other_job_type_child_male",
                                "other_job_type_child_female")
  ...
  data_cp_prep <- data_cp_prep %>%
    mutate(
      ...
      nb_children_working = rowSums(.[children_working_colmuns], na.rm = TRUE),
  ...
  data_cp_prep %>% mutate(
    ...
    CP_idicator1 = case_when(
      ## No Service
      cp_services_accesible == 0 ~ 4,
      
      ## 1 Services
      cp_services_accesible == 1 ~ 3,
...
compute_indicators_inter <- function(data, indicators) {
  
  df <- create_indicators_placeholder(data, indicators)
  
  data_inter_prep <- data
  ...
  data_inter_prep <- left_join(data_inter_prep, loops_data_washington %>% mutate_at(col_names,
                                                                                              .funs = ~ case_when(
                                                                                                .x %in% c("a_lot_difficulty", "cannot_do_at_all") ~ 1,
                                                                                                TRUE ~ 0
                                                                                              )
  ...
  data_inter_prep <- data_inter_prep %>% 
    mutate( 
      sent_abroad_hh = rowSums(.[grep("main_safety_concerns_.*\\.sent_abroad", names(.))], na.rm = TRUE),
      pysical_harresment_hh = rowSums(.[grep("main_safety_concerns_.*\\.pysical_harresment", names(.))], na.rm = TRUE),
      verbal_haresment_hh = rowSums(.[grep("main_safety_concerns_.*\\.verbal_haresment", names(.))], na.rm = TRUE),
  ...
  binary_gbv_cols <- mutate_at(data_inter_prep,gbv_cols,~ case_when(
    .x == "yes" ~ 1,
    TRUE ~ 0
  )) %>% rename_at(gbv_cols,~ paste0(.x,"_int")) %>% select(uuid,all_of(paste0(gbv_cols,"_int"))) 
  
  data_inter_prep <- left_join(data_inter_prep,binary_gbv_cols)
  ...
  data_inter_prep <- left_join(data_inter_prep, shelters_repeat %>% select(index=`_parent_index`,avg_size_shelter) )
  
  data_inter_prep$avg_size_shelter[is.na(data_inter_prep$avg_size_shelter)] <- 0
  ...
  data_inter_prep <- left_join(data_inter_prep, shelters_repeat_2 )
  ...
  data_inter_prep %>% mutate(
    #### Intersectoral 1 ####
    inter_idicator1 = case_when(
      ## Insecurity On Route To Points Of Aid DistributionInsecurity At Site Of Aid DistributionExclusion By Camp Managers/Gatekeepers
      aid_barriers.insecurity_route == 1 |
        aid_barriers.insecurity_site == 1 |
        aid_barriers.exclusion == 1 ~ 4,
    ...
    inter_idicator6 = case_when(
      risk_eviction == "yes"  & land_dispute == "yes" &
        land_dispute_yes.multiple_claims == 1 ~ 5, 

      risk_eviction == "yes"  & land_disputes_count >= 1 ~ 4,
      
      risk_eviction == "yes"  & land_disputes_count == 0 ~ 3,
    ...
    inter_idicator10 = case_when(
      ...
      sanitation_facility %in% no_improved_sanitation_facilities &
        sharing_sanitation_facilities %in% c("yes") &
        (sharing_sanitation_facilities_yes_int * 7) >=50 ~ 3,
      ###YS sharing_sanitation_facilities_yes is number of hh, not number of people
      ###AB One possible solution is to multiple the number of hh by the average size of hh but we need to validate this with the cluster
      ## HH: Access to unmproved sanitation facilities, shared with more than 20 people
      sanitation_facility %in% no_improved_sanitation_facilities &
  ...
  )  %>%  select(starts_with("inter_idicator")) -> res
